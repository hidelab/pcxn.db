...
# NOTES
# - edge width has been adjust to represent absolute correlation x10 (the multiplication is done in order for the edges to have visibly different width)
# - edge color is either blue (positive correlation) or red (negative correlation). The amount of correlation is represented by edge width
  ...
  if (df@type == "analyze") {
    ...
    for(i in 1:length(df@geneset_groups$top_correlated_genesets)) {
      tryCatch({
        ...
        print(paste("No edges containing ", cgs, " pass the correlation and p-value filters therefore the node is not included in the network",sep = ""))
    ...
    for(j in 1:length(df@geneset_groups$phenotype_0_genesets)) {
      tryCatch({
        ...
        print(paste("No edges containing ", cgs1, " pass the correlation and p-value filters therefore the node is not included in the network",sep = ""))
      })
    }
    for(h in 1:length(df@geneset_groups$phenotype_1_genesets)) {
      tryCatch({
        ...
        print(paste("No edges containing ", cgs2, " pass the correlation and p-value filters therefore the node is not included in the network",sep = ""))
      })
    }
  }

  # drawing the plot
  plot <- tkplot(graph, layout= layout_in_circle, width = NULL, height = NULL, edge.width = edge_thickness, edge.color = edge_color)
