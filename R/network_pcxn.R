...
#' @examples
#' \dontrun{
#' network <- network_pcxn(df)
#' }

network_pcxn <- function(df) {
    ...
    for(i in 1:length(df@geneset_groups$top_correlated_genesets)) {
      tryCatch({
        ...
        print(paste("No edges containing ", cgs, 
                    " pass the correlation and p-value filters therefore the 
                    node is not included in the network",sep = ""))
  ...
  # drawing the plot
  plot <- igraph::tkplot(graph, layout= igraph::layout_in_circle, width = NULL, height = NULL,
